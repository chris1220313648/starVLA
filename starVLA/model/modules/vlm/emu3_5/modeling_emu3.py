...
def _get_usable_cache_length(cache, new_seq_length, layer_idx=0):
    """Bridge the cache API used by the vendored model across Transformers versions."""
    if hasattr(cache, "get_usable_length"):
        return cache.get_usable_length(new_seq_length, layer_idx)
    cache_length = cache.get_seq_length(layer_idx)
    max_cache_length = cache.get_max_cache_shape(layer_idx)
    if max_cache_length is not None and cache_length + new_seq_length > max_cache_length:
        return max(0, max_cache_length - new_seq_length)
    return cache_length
